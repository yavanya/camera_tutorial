...
List<Image> computeImages(List<CameraImage> list) {
  ...
  var frontRaw = list.last;

  list.removeLast();

  var frontimage = convertImage(frontRaw);

  var bytes = imglib.encodeJpg(frontimage) as Uint8List;

  var frontImage = Image.memory(bytes);
  ...
  imageWidgets.add(gifWidget);

  imageWidgets.add(frontImage);

  return imageWidgets;
}

class LiveStaticViewModel extends FutureViewModel {
  LiveStaticViewModel(this.finalList, this.rawFront);
  final List<CameraImage> finalList;
  final CameraImage rawFront;
  ...
  Future<void> runCompute() async {
    // isComputing = true;
    List<CameraImage> list = finalList;
    list.add(rawFront);

    finalImages = await compute(computeImages, list);
